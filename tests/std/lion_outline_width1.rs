...
fn parse_lion() -> (Vec<agrega::Path>, Vec<agrega::Srgba8>) {
    ...
    let mut paths = vec![];
    let mut colors = vec![];
    let mut path = agrega::paths::Path::new();
    //let mut color = agrega::Srgba8::black();
    let mut color = agrega::color::Srgba8::new(0, 0, 0, 255);
    let mut cmd = agrega::paths::PathCommand::Stop;
    ...
    for line in txt.lines() {
        ...
        if v.len() == 1 {
            ...
            path = agrega::paths::Path::new();
            let rgb = agrega::color::Rgba8::new(r, g, b, 255);
...
fn lion_outline_width1() {
    ...
    agrega::render_all_paths(&mut ras, &mut ren, &stroke, &colors);
